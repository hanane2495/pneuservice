...
function SearchResultAg(){   
    ...
    useEffect(() => {
        ...
        axios.post(`${process.env.REACT_APP_API_URL}/agricole/search/pneus`, {
            largeur, hauteur, diametre, type, marque
        }).then(res => {
    ...
    return(
        <React.Fragment>
            <Navbar color='#262626'/>
            <Styles> 
    <h5 style={{fontWeight:'600', color:'#444'}}>Notre selection de pneus : {match.params.largeur}/{match.params.hauteur} R{match.params.diametre} {match.params.type} {match.params.position} {match.params.marque}</h5>
                <hr/>
                ...
                <div className='Right-side'>
                    ...
                    <Card_pneu>
                        ...
                        <p style={{fontSize :'1em', fontWeight:'600', color:'#444', margin:'0', padding:'0' }}>Position : {res.position}</p>
                        ...
                        <div className = 'card-info-4'>
                            <Link to={`/details/pneu/agricole/${res.pneu_id}/${res.designation_pneu}`} className='details' onClick={() => {handleDetails(JSON.stringify(res))}}>Details</Link>
                            {res.price > 0 ?  
