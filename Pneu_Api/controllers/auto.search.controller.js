...
/**
 * db.body('select pneu_id, designation_pneu, collection, type, marque, marque_img, largeur, hauteur, charge, vitesse, carburant, adherence, bruit, promo, COALESCE(marge,0) + COALESCE(prix,0) as price, image_url from pneu_dimension, stock, mapping_pneu_four where mapping_pneu_four.id_pneu_service = pneu_dimension.pneu_id and mapping_pneu_four.id_pneu_fournisseur = stock.id_supplier and largeur=$1 and hauteur=$2 and diametre=$3 and charge=$4 order by pneu_id ',
                          [req.query.largeur, req.body.hauteur, req.body.diametre, req.body.charge] ,
                          (err, results) => {
                               if(err){
                                   return res.send(err)
                               }else{
                                    var pneus1 = results.rows;
                                    var pneus2 = []
                                    db.query('select pneu_id, designation_pneu, type, collection, marque, marque_img, largeur, hauteur, charge, vitesse, carburant, adherence, bruit, promo, marge, image_url from pneu_dimension where largeur=$1 and hauteur=$2 and diametre=$3 and charge=$4', 
                                    [req.body.largeur, req.body.hauteur, req.body.diametre, req.body.charge],
                                    (err, results) =>{
                                        done()
                                        if(err){
                                            console.log(err)
                                        ...
                                        Array.prototype.push.apply(pneus1,pneus2)
                                        console.log(pneus1)
                                        console.log(req.body.largeur, req.body.hauteur, req.body.diametre, req.body.charge)
                                        return res.json(pneus1)
                                        }
